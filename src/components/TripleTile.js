...
const threeIndices = ["0", "1", "2"];

class TripleTile extends Component {
  constructor(props) {
    super(props);
    this.state = {
      xPosition: 0,
      yPosition: 0,
      width: 3 * this.props.tileHeight,
  ...
  handleClick(){
    ...
    if(!this.state.beingDragged){
      this.setState({
        width: this.state.width === 3 * this.props.tileHeight?this.props.tileHeight:3 * this.props.tileHeight,
        height: this.state.height === 3 * this.props.tileHeight?this.props.tileHeight:3 * this.props.tileHeight,
  ...
  updatePosition(x, y, index, overBoard){
    console.log(x, y, index, overBoard);
    let modX = x;
    let modY = y;;
    let xAdjust = 0;
    let yAdjust = 0;
